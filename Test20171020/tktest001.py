...
if __name__ == '__main__':
    ...
    sortedhouselist= (sorted(info_list,key=lambda x:x.split(' ')[1]))


    root = Tk()
    root.title("查看房价")
    root.geometry('300x300')
    # w1 = Label(root,text="点击查看的小区:\n"+reduce(lambda x,y:x+" "+y+"\n",houselist),background="green")
    w1 = Label(root,text="点击查看,显示小区价格\n",background="pink")
    w1.pack()
    def getinfo():
        for i in sortedhouselist:
            s=Label(root,text=i)
            s.pack()
    b1 = Button(root,text="查看",command=getinfo)
    b1.pack()
    root.mainloop()